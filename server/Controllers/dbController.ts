...
export const dbController = {
    ...
    write: async (request: Request, response: Response, next: NextFunction) => {
        ...
        const dbAsText = await fs.readFileSync(path.join(__dirname, '../../../server/db.json'), 'utf8')
        ...
        fs.writeFileSync(path.join(__dirname, '../../server/db.json'), JSON.stringify(dbAsArray));
